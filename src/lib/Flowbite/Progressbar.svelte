<script lang="ts">
  ...
  export let labelInsideClass: string =
    "text-white text-xs font-medium text-center p-0.5 leading-none rounded-full";
...
{#if labelOutside}
  <div
    {...$$restProps}
    class={classNames("flex justify-between mb-1", $$props.class)}
  >
    <span class="text-base font-medium text-primary-700 dark:text-white"
      >{labelOutside}</span
    >
...
>
  {#if labelInside}
    <div
      class={classNames(labelInsideClass, barColors[color])}
  ...
  {:else}
    <div
      class={classNames(barColors[color], size, "rounded-full")}
      style="width: {progress}%"
    />
  {/if}
</div>
